...
static Paragraph *Pp(ParagraphRoot *, Discount::Line *, int);
static Paragraph *compile(Discount::Line *, int, MMIOT *);
 ...
 */
static int
nextblank(Discount::Line *t, int i)
{
 ...
 */
static int
nextnonblank(Discount::Line *t, int i)
{
    while ( (i < S(t->text)) && isspace(T(t->text)[i]) )
	++i;
    return i;
 ...
 */
int
mkd_firstnonblank(Discount::Line *p)
{
...
static inline int
blankline(Discount::Line *p)
{
    return ! (p && (S(p->text) > p->dle) );
}


static Discount::Line *
skipempty(Discount::Line *p)
{
...
static void
splitline(Discount::Line *t, int cutpoint)
{
    if ( t && (cutpoint < S(t->text)) ) {
	Discount::Line *tmp = (Discount::Line *)calloc(1, sizeof *tmp);
...
static int
issetext(Discount::Line *t, int *htyp)
{
    Discount::Line *n;
...
static int
ishdr(Discount::Line *t, int *htyp)
{
...
static Discount::Line*
is_discount_dt(Discount::Line *t, int *clip)
{
#if USE_DISCOUNT_DL
    if ( t && t->next
	   && (S(t->text) > 2)
...
static Discount::Line*
is_extra_dt(Discount::Line *t, int *clip)
{
#if USE_EXTRA_DL
...
static Discount::Line*
isdefinition(Discount::Line *t, int *clip, int *kind)
{
    Discount::Line *ret;
...
static int
islist(Discount::Line *t, int *clip, DWORD flags, int *list_type)
{
    int i, j;
    char *q;
...
static int
centered(Discount::Line *first, Discount::Line *last)
{
...
static int
endoftextblock(Discount::Line *t, int toplevelblock, DWORD flags)
{
    int z;
...
{
    Discount::Line *t, *next;

    for ( t = p->text; t ; t = next ) {
	if ( ((next = t->next) == 0) || endoftextblock(next, toplevel, flags) ) {
	    p->align = (AlignType)centered(p->text, t);
	    t->next = 0;
	    return next;
	}
    }
...
static int
isdivmarker(Discount::Line *p, int start, DWORD flags)
{
    char *s;
    int last, i;
...
quoteblock(Paragraph *p, DWORD flags)
{
	...
	if ( p->ident = (char *)malloc(4+strlen(prefix)+S(q->text)) )
...
typedef int (*linefn)(Discount::Line *);
...
definition_block(Paragraph *top, int clip, MMIOT *f, int kind)
{
    ParagraphRoot d = { 0, 0 };
    Paragraph *p;
    Discount::Line *q = top->text, *text = 0, *labels; 
    int z, para;
 ...
 */
static Discount::Line*
addfootnote(Discount::Line *p, MMIOT* f)
{
    int j, i;
    int c;
    Discount::Line *np = p->next;

    Footnote *foot = &EXPAND_footnote(*f->footnotes);
 ...
 */
static Paragraph *
Pp(ParagraphRoot *d, Discount::Line *ptr, int typ)
{
    Paragraph *ret = (Paragraph *)calloc(sizeof *ret, 1);

    ret->text = ptr;
	ret->typ = (ParagraphType)typ;
...
static Discount::Line*
consume(Discount::Line *ptr, int *eaten)
{
    Discount::Line *next;
    int blanks=0;
 ...
 */
static Paragraph *
compile_document(Discount::Line *ptr, MMIOT *f)
{
    ParagraphRoot d = { 0, 0 };
    ANCHOR(Discount::Line) source = { 0, 0 };
    Paragraph *p = 0;
    struct kw *tag;
    int eaten, unclosed;
...
static int
first_nonblank_before(Discount::Line *j, int dle)
{
...
static int
actually_a_table(MMIOT *f, Discount::Line *pp)
{
    Discount::Line *r;
    int j;
    int c;
 ...
 */
static Paragraph *
compile(Discount::Line *ptr, int toplevel, MMIOT *f)
{
    ParagraphRoot d = { 0, 0 };
    Paragraph *p = 0;
    Discount::Line *r;
...
mkd_compile(Document *doc, DWORD flags)
{
    ...
    CREATE(doc->ctx->in);
    doc->ctx->footnotes = (Discount::Footnote *)malloc(sizeof doc->ctx->footnotes[0]);
    CREATE_footnote(*doc->ctx->footnotes);
