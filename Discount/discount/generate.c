...
Qchar(int c, MMIOT *f)
{
    ...
    if ( S(f->Q) == 0 ) {
	cur = &EXPAND_block(f->Q);
	memset(cur, 0, sizeof *cur);
		cur->b_type = block::bTEXT;
    }
    else
	cur = &T(f->Q)[S(f->Q)-1];
...
{
    block *p = &EXPAND_block(f->Q);

    memset(p, 0, sizeof *p);
    p->b_type = (c == '*') ? block::bSTAR : block::bUNDER;
    p->b_char = c;
    p->b_count = count;

    memset(&EXPAND_block(f->Q), 0, sizeof(block));
}
...
{
    return p->ref_prefix ? p->ref_prefix : (char *)"fn";
}
...
linkylinky(int image, MMIOT *f)
{
	...
	if ( peek(f,1) == '(' ) {
		...
		if ( ref = (Footnote *)bsearch(&key, T(*f->footnotes), S(*f->footnotes),
...
mangle(char *s, int len, MMIOT *f)
{
    while ( len-- > 0 ) {
	Qstring("&#", f);
	Qprintf(f, COINTOSS() ? (char *)"x%02x;" : (char *)"%02d;", *((unsigned char*)(s++)) );
    }
}
...
text(MMIOT *f)
{
	...
	case 3:     Qstring(tag_text(f) ? (char *)"  " : (char *)"<br/>", f);
		    break;
...
static int
splat(Discount::Line *p, char *block, Istring align, int force, MMIOT *f)
{
    int first,
	idx = p->dle,
	colno = 0;
...
static void
printcode(Discount::Line *t, MMIOT *f)
{
    int blanks;
...
static void
printhtml(Discount::Line *t, MMIOT *f)
{
    int blanks;
...
htmlify(Paragraph *p, char *block, char *arguments, MMIOT *f)
{
    ___mkd_emblock(f);
    if ( block )
	Qprintf(f, arguments ? (char *)"<%s %s>" : (char *)"<%s>", block, arguments);
    ___mkd_emblock(f);
...
display(Paragraph *p, MMIOT *f)
{
    ...
    case QUOTE:
	htmlify(p->down, p->ident ? (char *)"div" : (char *)"blockquote", p->ident, f);
	break;
