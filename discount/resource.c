...
___mkd_initmmiot(MMIOT *f, void *footnotes)
{
	...
	CREATE_block(f->Q);
	if ( footnotes )
	    f->footnotes = (Discount::Footnote*)footnotes;
	else {
	    f->footnotes = (Discount::Footnote*)malloc(sizeof f->footnotes[0]);
	    CREATE_footnote(*f->footnotes);
	}
    }
}
