...
mkd_search_tags(char *pat, int len)
{
    ...
    if ( (ret=(kw*)bsearch(&key,blocktags,NR_blocktags,sizeof key,(stfu)casort)) )
	return ret;

    if ( S(extratags) )
	return (kw*)bsearch(&key,T(extratags),S(extratags),sizeof key,(stfu)casort);
