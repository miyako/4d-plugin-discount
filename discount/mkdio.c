...
typedef ANCHOR(Discount::Line) LineAnchor;
...
{
	Discount::Line *p = (Discount::Line *)calloc(sizeof *p, 1);
    unsigned char c;
    int xp = 0;
    int           size = S(*line);
 ...
 */
void
__mkd_header_dle(Discount::Line *p)
{
    CLIP(p->text, 0, 1);
    p->dle = mkd_firstnonblank(p);
}
...
populate(getc_func getc, void* ctx, int flags)
{
    ...
    if ( (pandoc == 3) && !(flags & (MKD_NOHEADER|MKD_STRICT)) ) {
		 ...
		 * off header.
		 */
		Discount::Line *headers = T(a->content);
